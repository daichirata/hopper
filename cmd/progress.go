...
func (r *reporter) clear(table string, deleted int64, done bool) {
	...
	case deleted == 0:
		fmt.Fprintf(r.w, "\rClearing %s...\033[K", table)
	default:
		fmt.Fprintf(r.w, "\rClearing %s... %d deleted\033[K", table, deleted)
	}
}
...
func (r *reporter) load(table string, done, total int) {
	...
	fmt.Fprintf(r.w, "\rLoading %-16s [%s] %3.0f%%  (%d/%d)", table, progressBar(ratio), ratio*100, done, total)
	if done >= total {
		fmt.Fprintln(r.w)
	}
}
